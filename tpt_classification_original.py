...
from data.transforms import GroupFullResSample
...
def test_time_tuning(model, inputs, optimizer, scaler, args):
    ...
    selected_idx = None
    for j in range(args.tta_steps): #1 step
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if isinstance(images, list): #true
            for k in range(len(images)): #64 images for tpt
                images[k] = images[k].cuda(args.gpu, non_blocking=True)
            image = images[0] #look at the first image of image list and use this for inference
        else: #false
        ...
        if not args.cocoop: # no need to reset cocoop because it's fixed #true
            if args.tta_steps > 0: #true
                with torch.no_grad():
                    model.reset()
            optimizer.load_state_dict(optim_state) 
            test_time_tuning(model, images, optimizer, scaler, args) #tune here
        else:
            with torch.no_grad():
                with torch.cuda.amp.autocast():
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                if args.cocoop:
                    ...
                    output = model(image) #inference here
