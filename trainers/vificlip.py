...
from clip import vificlip
from clip.simple_tokenizer import SimpleTokenizer as _Tokenizer

_tokenizer = _Tokenizer()


def load_vificlip_to_cpu(cfg):
    backbone_name = cfg.MODEL.ARCH
    url = vificlip._MODELS[backbone_name]
    model_path = vificlip._download(url)
    ...
    model = vificlip.build_model(state_dict or model.state_dict(), design_details)

    return model


class TextEncoder(nn.Module):
    def __init__(self, vificlip_model):
        super().__init__()
        self.transformer = vificlip_model.transformer
        self.positional_embedding = vificlip_model.positional_embedding
        self.ln_final = vificlip_model.ln_final
        self.text_projection = vificlip_model.text_projection
        self.dtype = vificlip_model.dtype
...
class VLPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, vificlip_model, logger):
        super().__init__()
        dtype = vificlip_model.dtype
        ...
        if ZS_evaluation:
            text_aug = f"{{}}"
            tokenized_prompts = torch.cat([vificlip.tokenize(text_aug.format(c), context_length=77) for c in classnames])
            embedding = vificlip_model.token_embedding(tokenized_prompts).type(dtype).cuda()
            ...
            ctx_dim = vificlip_model.ln_final.weight.shape[0]

            if ctx_init and (n_ctx) <= 4:
                # use given words to initialize context vectors
                ctx_init = ctx_init.replace("_", " ")
                n_ctx = n_ctx
                prompt = vificlip.tokenize(ctx_init)
                with torch.no_grad():
                    embedding = vificlip_model.token_embedding(prompt).type(dtype)
            ...
            tokenized_prompts = torch.cat([vificlip.tokenize(p) for p in prompts])  # (n_cls, n_tkn)
            with torch.no_grad():
                embedding = vificlip_model.token_embedding(tokenized_prompts).type(dtype)
...
class ViFiCLIP(nn.Module):
    def __init__(self, cfg, classnames, vificlip_model, logger):
        super().__init__()
        self.prompt_learner = VLPromptLearner(cfg, classnames, vificlip_model, logger)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.image_encoder = vificlip_model.visual
        self.text_encoder = TextEncoder(vificlip_model)
        self.logit_scale = vificlip_model.logit_scale
        self.dtype = vificlip_model.dtype
...
def returnCLIP(config, logger=None,
    ...
    vificlip_model = load_vificlip_to_cpu(config)

    logger.info("Building ViFi-CLIP CLIP")
    model = ViFiCLIP(config, class_names, vificlip_model, logger)
    ...
    if config.TRAINER.ViFi_CLIP.PROMPT_MODEL:
        ...
        train_complete_vificlip = config.TRAINER.ViFi_CLIP.USE
        if train_complete_vificlip == "both":
            logger.info("Turning on gradients for COMPLETE ViFi-CLIP model")
            for name, param in model.named_parameters():
                param.requires_grad_(True)
        else:
            if train_complete_vificlip == "image":
