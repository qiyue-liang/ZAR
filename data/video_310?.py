...
class Video_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.modality == 'video':
            ...
            for i_try in range(_num_retries):
                ...
                if self.root_path.split('/')[-2] == 'k600':
                    directory = os.path.join(self.root_path, 'val', record.path)
                    if not os.path.exists(directory):
                        directory = os.path.join(self.root_path, 'test', record.path)
                    if not os.path.exists(directory):
                        directory = os.path.join(self.root_path, 'train', record.path)
                else:
    ...
    def get(self, record, video_list, indices):
        images = list()
        for seg_ind in indices: #get indices
            p = int(seg_ind)
            if self.modality == 'video':
